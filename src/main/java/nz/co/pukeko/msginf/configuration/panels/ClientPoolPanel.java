...
public class ClientPoolPanel extends JPanel {
	private final JCheckBox useConnectionPooling = new JCheckBox("Use Connection Pooling", true);
	private final JLabel minimumConnectionsLabel = new JLabel("Minimum Number of Connections:");
	private final JSpinner minimumConnections = new JSpinner(new SpinnerNumberModel(5,1,10,1));
	private final JLabel maximumConnectionsLabel = new JLabel("Maximum Number of Connections:");
	private final JSpinner maximumConnections = new JSpinner(new SpinnerNumberModel(50,10,50,1));
	...
	private void init() {
	    ...
	    TitledBorder titledBorder = new TitledBorder(border, "Client Pool");
	...
	private int getMinimumConnections() {
		return (Integer) minimumConnections.getValue();
	}
	
	private void setMinimumConnections(int minConnections) {
		minimumConnections.setValue(minConnections);
	}
	
	private int getMaximumConnections() {
		return (Integer) maximumConnections.getValue();
	}

	private void setMaximumConnections(int maxConnections) {
		maximumConnections.setValue(maxConnections);
	}

...
public class ViewerSplitPanel extends JPanel {
	...
	private final JTabbedPane queueTabbedPane = new JTabbedPane();
	private final JLabel statusLabel = new JLabel("OK");
	...
	private void init() {
		...
		JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, treeScrollPane, queueTabbedPane);
	 ...
	 */
	public void displayMessagesInTab(MessageQueue messageQueue, List<Message> messages) throws JMSException {
		...
		if (doesTabExist(tabName)) {
			...
			if (tabIndex != null) {
				queueTabbedPane.remove(tabIndex);
			}
		}

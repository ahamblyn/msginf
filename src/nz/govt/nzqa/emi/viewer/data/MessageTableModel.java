package nz.govt.nzqa.emi.viewer.data;
	...
	private void updateTableData(Message[] data) throws JMSException {
		...
		for (int i = 0; i < data.length; i++) {
			...
			if (o instanceof Message) {
				...
				table_data[i][1] = msg.getJMSCorrelationID();
				table_data[i][2] = msg.getJMSMessageID();
				if (msg.getJMSDestination() != null) {
					table_data[i][3] = msg.getJMSDestination().toString();
				}
				if (msg.getJMSReplyTo() != null) {
					table_data[i][4] = msg.getJMSReplyTo().toString();
				}
				table_data[i][5] = msg.getJMSType();
				Date msgTS = new Date(msg.getJMSTimestamp());

...
import nz.govt.nzqa.emi.infrastructure.exception.XMLPropertiesFileException;
import nz.govt.nzqa.emi.infrastructure.pref.xmlbeans.ConnectorsDocument.Connectors;
import nz.govt.nzqa.emi.infrastructure.pref.xmlbeans.JarFileDocument.JarFile;
...
public class XMLMessageInfrastructurePropertiesFileParser {
	...
	/**
	 * This constructor only allows the <emi:configuration> element and the number of messaging systems configured to be accessed. No other system information can be accessed.
	 * @param parseDefaultFile whether to parse the default file defined in the emi.xmlpropertiesfile system property. If false then create a blank configuration.
	 * @throws MessageException
	 */
	public XMLMessageInfrastructurePropertiesFileParser(boolean parseDefaultFile) throws MessageException {
		...
		String baseXML = "<emi:configuration emi:log4jPropertiesFile=\"\" xmlns:emi=\"http://nzqa.govt.nz/emi/infrastructure/pref/xmlbeans\"/>";
		if (parseDefaultFile) {
			parseFile();
		} else {
			try {
	...
	private void parseFile() throws MessageException {
		File file = null;
		// get the XML properties file name from the system properties: -Demi.xmlpropertiesfile
		String fileName = java.lang.System.getProperty("emi.xmlpropertiesfile");
